...
function updateState(currentState, updates) {
  ...
  if (updates.series) {
    Object.assign(newState.series, updates.series);
  }
  ...
  for (const alert of newAlerts) {
    ...
    if (!shouldSuppress) {
      const processedAlert = {
        ...
        alert_id: alertId,
        severity,
        confidence: parseFloat(confidence.toFixed(2)), // Always numeric, 2 decimal places
        durationIntervals,
...
function updateStateFromInterval(prevState, intervalResult) {
  const { computeBaseline } = require('./watch-analytics');
  
  // If interval failed, return state with error info (no state change except performance)
  ...
  if (isFine && integrity.valid && prevState.baseline.status === 'established') {
    const { detectDrift } = require('./watch-analytics');
  ...
  if (roles && roles.roles) {
    const { detectRoleChanges } = require('./watch-analytics');
  ...
  if (isFine) {
    const { detectStructuralAlerts } = require('./watch-analytics');
